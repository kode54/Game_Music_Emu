...
public:
	...
	// emulation
	void reset();
	enum { start_addr = 0x4040 };
	enum { end_addr = 0x4092 };
	enum { reg_count = end_addr - start_addr + 1 };
...
private:
	...
	unsigned char regs_ [reg_count];// last written value to registers
	...
	unsigned char& regs( unsigned addr ) { return regs_ [addr - start_addr]; }
...
{
	synth.volume( 0.13 / master_vol_max / vol_max / wave_sample_max * v );
}

inline void Nes_Fds_Apu::osc_output( int index, Blip_Buffer* b )
{
